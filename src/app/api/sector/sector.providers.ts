...
export class SectorService {
  ...
  getVehicles(sector: Sector, query: Object = {}): Observable<any> {
    let queryString = Object.keys(query).map(k => `${encodeURIComponent(k)}=${encodeURIComponent(query[k])}`).join('&');

    return this.authHttp.get(`${environment.API_BASEURL}/api/sectors/${sector._id}/vehicles${queryString ? '?' + queryString : ''}`)
      .map(res => {
        if (!query['paging']) { return <Register[]>res.json(); }
